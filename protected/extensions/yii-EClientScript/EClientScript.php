...
class EClientScript extends CClientScript
{
	...
	/**
	 * @var when optimized script files already exist. 'optimizeScriptFiles' must be false and 'optimizedScriptFileSuffix' must be defined.
	 */
	public $useOptimizedScriptFiles = true;

	/**
	 * @var when optimized script files already exist. Define minimized file extension.For example "jquery.js" and "jquery.min.js" when variable is set to "min"
	 */
	public $optimizedScriptFileSuffix = 'min';
	...
	{
		if ($this->useOptimizedScriptFiles)
		{
			$url_parts = pathinfo($url);
			$url = $url_parts['dirname'].'/'.$url_parts['filename'].'.'.$this->optimizedScriptFileSuffix.'.'.$url_parts['extension'];
		}
