...
class DefaultController extends UsrController
{
    ...
    {
        return array(
            array('allow', 'actions'=>array('register'), 'expression'=>'!$user->isGuest'), //'roles'=>array('admin')),
            /*array('allow', 'actions'=>array('index', 'logout', 'profile', ), 'expression'=>'!$user->isGuest'),
            array('allow', 'actions'=>array('recovery', 'verify', 'login', ), 'users'=>array('*')),

            array('deny', 'users'=>array('*')), // deny all users*/
        );
    }
	...
	public function beforeAction($action)
	{
		...
		case 'register':
			...
			if ((!$this->module->registrationEnabled) && (!Yii::app()->user->checkAccess('admin'))) {
	...
	{
        if ($this->module->afterLoginStaticAction)
        {
            if (strpos(Yii::app()->user->returnUrl, $this->module->afterLoginUrl) === false)
                $this->redirect(Yii::app()->createUrl($this->module->afterLoginUrl));
            else
